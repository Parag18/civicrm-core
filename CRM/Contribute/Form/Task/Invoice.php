...
class CRM_Contribute_Form_Task_Invoice extends CRM_Contribute_Form_Task {
  ...
  public function buildQuickForm() {
    ...
    $this->addElement('radio', 'output', NULL, ts('Email Invoice'), 'email_invoice', 
      array('onClick' => "document.getElementById('selectPdfFormat').style.display = 'none';document.getElementById ('comment').style.display = 'block'; document.getElementById ('emailId').style.display = 'block';")
    );
    $this->addElement('radio', 'output', NULL, ts('PDF Invoice'), 'pdf_invoice',
      array('onClick' => "document.getElementById('comment').style.display = 'none';document.getElementById ('emailId').style.display = 'none';document.getElementById('selectPdfFormat').style.display = 'block';")
    );
   ...
   * 
   */
  public function printPDF($contribIDs, $params, $contactIds) {
    ...
    foreach ($invoiceElements['details'] as $contribID => $detail) {
      ...
      if ($contribution->_component == 'event') {
        ...
        if (!empty($mailDetails[$contribution->contribution_page_id]['receipt_from_name']) || !empty($mailDetails[$contribution->contribution_page_id]['receipt_from_email']) ||
            !empty($mailDetails[$contribution->contribution_page_id]['cc_receipt']) || !empty($mailDetails[$contribution->contribution_page_id]['bcc_receipt'])) {
        ...
        if (!empty($mailDetails[$contribution->contribution_page_id]['title'])) {
      ...
      if (!empty($billingAddress[$contribution->contact_id]['street_address']) || !empty($billingAddress[$contribution->contact_id]['supplemental_address_1']) || 
          !empty($billingAddress[$contribution->contact_id]['supplemental_address_2']) || !empty($billingAddress[$contribution->contact_id]['city']) || 
          !empty($billingAddress[$contribution->contact_id]['city']) || !empty($stateProvinceAbbreviation) || !empty($billingAddress[$contribution->contact_id]['postal_code'])) {
        $streetAddress = CRM_Utils_Array::value('street_address', $billingAddress[$contribution->contact_id]);
        $supplementalAddress_1 = CRM_Utils_Array::value('supplemental_address_1', $billingAddress[$contribution->contact_id]);
        $supplementalAddress_2 = CRM_Utils_Array::value('supplemental_address_2', $billingAddress[$contribution->contact_id]);
        $city = CRM_Utils_Array::value('city', $billingAddress[$contribution->contact_id]);
        $provinceAbbreviation = $stateProvinceAbbreviation;
        $postalCode = CRM_Utils_Array::value('postal_code', $billingAddress[$contribution->contact_id]);
      } else {
        $streetAddress = '';
        $supplementalAddress_1 = '';
        $supplementalAddress_2 = ''; 
        $city = '';
      ...
      $tplParams = array(
                         ...
                         'city' => $city,
                         'stateProvinceAbbreviation' => $provinceAbbreviation,
      ...
      elseif ($contribution->_component == 'contribute') {
        ...
        self::addActivities($subject, $contribution->contact_id, $fileName, $params['output']);
      }
 ...
 /**
   ...
   * 
   */
  public function addActivities($subject, $contactIds, $fileName, $output) {
    $session        = CRM_Core_Session::singleton();
    $userID         = $session->get('userID');
    $config = CRM_Core_Config::singleton();
    $config->doNotAttachPDFReceipt = 1;
    if ($output == 'email_invoice') {
    ...
    $activityParams = array(
      ...
      'attachFile_1' => array ('uri' => $fileName, 
